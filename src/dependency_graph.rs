//! Directed dependency graph built on `petgraph`. Tracks files, their types, output
//! locations, and import relationships. Used to determine transformation order and
//! compute relative paths between output files.
...
pub enum TargetLocation {
    /// for the specific component folder
    Component(String),
    /// Global CSS folder
    CssGlobal,
    /// Asset file folder
...
impl TargetLocation {
    /// Returns the output directory name for this target location, or None for Omit.
    pub fn dir_name(&self) -> Option<&str> {
        match self {
            TargetLocation::Component(_) => Some("components"),
            TargetLocation::CssGlobal => Some("styles"),
            TargetLocation::Asset => Some("assets"),
            TargetLocation::Dependency => Some("dependencies"),
            TargetLocation::Omit => None,
        }
    }

    /// Returns all non-Omit directory names used for output.
    pub fn all_dir_names() -> &'static [&'static str] {
        &["components", "styles", "assets", "dependencies"]
    }
}
...
impl FileNode {
    ...
    pub fn get_dist_path(&self) -> Option<PathBuf> {
        let dir_name = self.target_location.dir_name()?;
        let filename = self.path.file_name()?;

        let base = PathBuf::from(dir_name);
        match &self.target_location {
            TargetLocation::Component(name) => Some(base.join(name).join(filename)),
            _ => Some(base.join(filename)),
        }
    }
}
...
impl DependencyGraph {
    ...
    pub fn add_dependency(
        ...
        let from_idx = self
            .path_to_index
            .get(from_file)
            .ok_or_else(|| DependencyGraphError::SourceFileNotFound(from_file.to_path_buf()))?;

        let to_idx = self
            .path_to_index
            .get(to_file)
            .ok_or_else(|| DependencyGraphError::TargetFileNotFound(to_file.to_path_buf()))?;
    ...
    /// Promote a file's target location if it is currently Omit.
    /// This is used when a non-component file imports an omitted dependency,
    /// meaning the dependency should be included in output.
    pub fn promote_target_location(&mut self, path: &Path, new_location: TargetLocation) {
        if let Some(&idx) = self.path_to_index.get(path)
            && self.graph[idx].target_location == TargetLocation::Omit
        {
            self.graph[idx].target_location = new_location;
        }
    }

    /// Get a file node by its path, if it exists in the graph.
    pub fn get_file(&self, file_path: &Path) -> Option<&FileNode> {
        self.path_to_index
            .get(file_path)
    ...
    pub fn get_file_dependencies(
        ...
        let node_idx = self
            .path_to_index
            .get(file_path)
            .ok_or_else(|| DependencyGraphError::FileNotFound(file_path.to_path_buf()))?;
    ...
    /// Build a global map of all chrome:// import URLs to their dist-relative paths.
    /// This maps every chrome:// URL in the entire graph (from all edges) to the
    /// dist path of the target file. Used for replacing chrome:// URLs that aren't
    /// direct dependencies of the file being transformed.
    pub fn build_global_chrome_url_map(&self) -> HashMap<String, PathBuf> {
        let mut map = HashMap::new();

        for edge_idx in self.graph.edge_indices() {
            let edge = &self.graph[edge_idx];
            let import_stmt = &edge.import_statement;

            // Only include chrome:// URLs
            if !import_stmt.starts_with("chrome://") {
                continue;
            }

            let target_idx = self.graph.edge_endpoints(edge_idx).unwrap().1;
            let target_node = &self.graph[target_idx];

            if let Some(dist_path) = target_node.get_dist_path() {
                map.insert(import_stmt.clone(), dist_path);
            }
        }
    ...
    pub(crate) fn get_css_imports(&self, path: &Path) -> Vec<(String, PathBuf)> {

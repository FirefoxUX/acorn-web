//! Parses Firefox `jar.mn` manifest files and `moz.build` content-accessible declarations
//! to build a map from `chrome://` and `resource://` URLs to filesystem paths.
...
impl JarResolver {
    ...
    pub fn new(
        ...
        default_ifdef_config.insert("MOZ_GECKOVIEW".to_string(), false);
...
fn parse_file_line(
    ...
    // Handle wildcard source patterns (e.g., "content/assets/*")
    if let Some(src) = source
        && src.contains('*')
    {
        return expand_wildcard_mapping(
            destination,
            src,
            jar_dir,
            firefox_dir,
...
/// Expands a wildcard source pattern (e.g., `content/assets/*`) into individual file mappings.
fn expand_wildcard_mapping(
    destination: &str,
    source_pattern: &str,
    jar_dir: &Path,
    firefox_dir: &Path,
    ...
    // Replace the wildcard to get the source directory
    let source_dir_str = source_pattern.trim_end_matches('*').trim_end_matches('/');
    let source_dir = if source_dir_str.starts_with('/') {
        PathBuf::from(source_dir_str.strip_prefix('/').unwrap_or(source_dir_str))
    } else {
        jar_dir.join(source_dir_str)
    };

    let full_source_dir = firefox_dir.join(&source_dir);

    if !full_source_dir.is_dir() {
        return Ok(()); // Directory doesn't exist, skip silently
    }

    let entries = fs::read_dir(&full_source_dir)?;
    for entry in entries {
        ...
        if let Some(filename) = entry.file_name().to_str() {
            // Build destination + filename (destination ends with '/' for directory mappings)
            let dest_base = destination.trim_end_matches('/');
            let file_destination = format!("{}/{}", dest_base, filename);

            if let Some(chrome_url) = build_chrome_url(&file_destination, chrome_registrations) {

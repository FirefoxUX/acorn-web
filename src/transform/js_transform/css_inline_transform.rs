//! Inlines external CSS `<link>` tags into component classes as a static `styles` property.
//! Finds `<link rel="stylesheet" href="...">` in `html``\` templates, removes them, and
//! generates `static styles = [...(Super.styles ?? []), css\`…\`]` with the inlined CSS.
...
impl<'a> CssInlineTransformer<'a> {
    ...
    /// Traverses the program AST looking for classes with `html``\`` templates that
    /// contain stylesheet `<link>` tags. Returns `true` if any CSS was inlined.
    pub fn build(
        &mut self,
...
impl<'a> Traverse<'a, ()> for CssInlineTransformer<'a> {
    fn enter_class(&mut self, class: &mut ast::Class<'a>, ctx: &mut TraverseCtx<'a, ()>) {
        ...
        let super_class_name_string =
            if let Some(ast::Expression::Identifier(ident)) = class.super_class.as_ref() {
                Some(ident.name.to_string())
            } else {
                None
            };
        let super_class_name = super_class_name_string.as_deref();
...
impl<'a> CssInlineTransformer<'a> {
    ...
    /// Scans an `html``\`` template literal for `<link rel="stylesheet" href="...">` tags.
    /// For each link whose href matches a CSS replacement, removes the tag from the template
    /// and records the href so `add_styles_property` can inline the CSS content later.
    ...
    fn add_styles_property(
        ...
        if new_properties.iter().any(|prop| {
            if let ClassElement::PropertyDefinition(prop_def) = prop
                && let PropertyKey::Identifier(ident) = &prop_def.key
            {
        ...
        if !combined_css.is_empty() {
            // --- Build `css`...`` tagged template with the inlined CSS content ---
            ...
            // --- Build the array: [...(SuperClass.styles ?? []), css`...`] ---
            // The spread inherits parent styles, and `?? []` guards against undefined.
            // oxc's arena allocator (`ctx.ast.alloc`, `ctx.ast.vec_with_capacity`) owns all nodes.
            ...
            // --- Build `static styles = [...]` property definition ---

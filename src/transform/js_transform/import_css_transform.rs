//! Ensures the `css` tagged template function is imported from `lit.all.mjs` when
//! CSS inlining has added `css``\` expressions to a component file.
...
/// Ensures `css` is included in the `lit.all.mjs` import specifiers.
/// Run after `CssInlineTransformer` has added `css``\`` expressions that need
/// the `css` tagged template function to be in scope.
...
impl ImportCssTransformer {
    ...
    /// Traverses the program AST, adding `css` to any `lit.all.mjs` import if not already present.
    pub fn build<'a>(
        &mut self,
...
impl<'a> Traverse<'a, ()> for ImportCssTransformer {
    fn enter_import_declaration(
        ...
        let css_already_imported = node.specifiers.as_ref().is_some_and(|specs| {
            specs.iter().any(|spec| {
